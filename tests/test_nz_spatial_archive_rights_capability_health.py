...
def _record() -> dict:
    return json.loads(
        (ROOT / "docs/nz-spatial-archive-rights-capability-health-20260830.json").read_text()
    )
...
def test_archive_only_record_is_fail_closed() -> None:
    ...
    assert (
        record["disposition"]["network_timetable_facility_national_authoritative_claims_enabled"]
        is False
    )
...
def test_archive_only_record_matches_schema() -> None:
    record = _record()
    schema = json.loads(
        (ROOT / "schemas/nz-spatial-archive-rights-capability-health.schema.json").read_text()
    )
    assert (
        list(Draft202012Validator(schema, format_checker=FormatChecker()).iter_errors(record)) == []
    )
