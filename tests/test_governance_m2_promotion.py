...
TRACK = ROOT / "conductor/tracks/governance_maori_data_sovereignty_20260718"
RECEIPT = ROOT / "docs/governance-m2-promotion-20260826.json"


def test_governance_m2_promotion_is_exact_tree_and_fail_closed() -> None:
...
def test_programme_registry_reports_governance_m2_only() -> None:
    tracks = (ROOT / "conductor/tracks.md").read_text(encoding="utf-8")
    governance_line = next(
        line
        for line in tracks.splitlines()
        if "`governance_maori_data_sovereignty_20260718`" in line
    )
    assert "current `M2`" in governance_line
    assert "target `M6`" in governance_line
