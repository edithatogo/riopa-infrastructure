...
def test_canonical_candidate_freeze_is_digest_bound_and_fail_closed() -> None:
    ...
    record = json.loads((root / "docs/canonical-v1-candidate-freeze-20260825.json").read_text())
    ...
    assert "SHACL engine execution and report" in record["open_gates"]
    assert "persistent publication identifier and authority decision" in record["open_gates"]
