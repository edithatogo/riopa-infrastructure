...
| Hamilton food-register packet | A under the exact ArcGIS item's CC BY 4.0 licence; B only if a successor item loses that binding. | **A now.** Keep the exact-version payload public with Hamilton City Council attribution and the item-specified derivative notice. | Quarantine only affected material if exact terms change or third-party content is identified. |
| Marlborough food-register packet | A under the Council Open Data default of CC BY 3.0 NZ because the exact OpenData layer has no contrary notice; B if an exception is later found. | **A now.** Keep the exact-version payload public with Marlborough District Council attribution. | Apply any later item-specific exception and quarantine only affected material. |
