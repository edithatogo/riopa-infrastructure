...
|---|---|---|---|
| Normative inventory and conformance | `scripts/conformance_node.mjs`, `tests/test_conformance.py`, `tests/test_canonical.py`, `docs/ontology/canonical-conformance-manifest-1.0.0.json` | Frozen inventory, complete corpus, cross-language outputs and reviewer sign-off | Freeze affected vocabulary; fail closed on unmapped fields and narrow claims. |
...
| External user/operator workflows | `docs/wp010-external-reproduction-handoff.md`, `docs/wp010-external-reproduction-report-template.md` | Two user workflows and one operator workflow with environment, deviations, adverse findings and disposition | Keep preview label; do not infer usability or operational fitness from internal agents. |
