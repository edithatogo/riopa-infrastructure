...
| `FACILITY-PANEL-MANIFEST-HARDENING-20260829` | Four-lens cardinality, UTC timestamp and safe unique artifact-path checks for the future panel manifest | `scripts/validate_facility_panel_manifest.py`, `tests/test_facility_panel_manifest.py` | Shape hardening is complete; referenced bytes and factual adjudication still require a future content-bound rerun |
...
| `FACILITY-SNAPSHOT-NESTED-VALIDATION-20260829` | Fail-closed validation of nested public assertion rows and exclusion ledger in snapshot candidates | `src/riopa_provenance/facility_registry.py::validate_snapshot_record`, `docs/facility-snapshot-nested-validation-20260829.json`, `tests/test_facility_registry.py` | Malformed rows, duplicate identities, non-public rows and malformed ledgers are rejected locally; authority, completeness, publication and release approval remain open |
