...
| `WP006-PROFILE-VALIDATOR-INVENTORY-20260829` | Machine-checked coverage inventory for every claimed publication profile | `scripts/validate_profile_validator_inventory.py`, `docs/wp006-profile-validator-inventory-20260829.json`, `tests/test_profile_validator_inventory.py` | Inventory completeness passes; bounded/external statuses are explicit, while external producer/consumer conformance and trusted signing remain open |
| `WP006-PROFILE-VALIDATOR-INVENTORY-HARDENING-20260829` | Closed status vocabulary and mandatory boundary declarations prevent external-certification overclaims | `scripts/validate_profile_validator_inventory.py`, `tests/test_profile_validator_inventory.py` | Overclaim and missing-boundary negatives pass; no external certification is asserted |
| `WP006-RELEASE-CONFORMANCE-REPORT-VALIDATION-20260829` | Machine-check the technical-preview report's revision, fixture and evidence bindings | `scripts/validate_release_conformance_report.py`, `docs/release-conformance-report-validation-20260829.json`, `tests/test_release_conformance_report.py`, `.github/workflows/release.yml` | Repository validator and negative tests pass; signing, provider acceptance, external execution and authority remain open |
| `WP006-V040-PRESERVATION-RECEIPT-VALIDATION-20260829` | Deterministically validate the v0.4.0 successor preservation receipt chain and exact local digests | `scripts/validate_v040_preservation_receipts.py`, `docs/v040-preservation-receipt-validation-20260829.json`, `tests/test_v040_preservation_receipt_validator.py` | Local receipt-chain validation passes; provider restore, stable-candidate preservation and authority remain open |
| `WP006-RELEASE-REPORT-FIXTURE-BINDING-20260829` | Verify the report fixture digest against the checked-in client-workflow bytes | `scripts/validate_release_conformance_report.py`, `tests/test_release_conformance_report.py`, `docs/wp006-release-report-fixture-binding-20260829.json` | Tampered fixture digest fails closed; stable signing, preservation and authority remain open |
| `WP006-RUST-SCHEMA-RFC8785-PARITY-20260829` | Execute one shared canonical and schema corpus through Python, Node and Rust | `conformance/v1/corpus.json`, `rust/riopa-conformance/`, `docs/ontology/interoperability-compatibility-matrix-20260829.json`, `tests/test_rust_producer_consumer.py` | Seven cases pass in all three implementations; Rust now validates Draft 2020-12 schema outcomes and RFC 8785 numeric and UTF-16 ordering vectors |
| `CANONICAL-V1-BOUNDED-FREEZE-20260829` | Supersede the earlier bounded freeze after expanding the normative conformance corpus | `docs/canonical-v1-candidate-freeze-20260829.json`, `tests/test_canonical_candidate_freeze.py` | Digest-bound bounded candidate only; it does not freeze the stable candidate or start beta/RC clocks |
| `WP006-CORPUS-ENVELOPE-VALIDATION-20260829` | Validate the normative corpus envelope before cross-runtime parity execution | `scripts/verify_conformance_parity.py`, `tests/test_conformance.py`, `tests/test_canonical.py` | Malformed corpus metadata and references fail before parity; external standards acceptance remains open |
| `WP006-CORPUS-CASE-CLASS-VALIDATION-20260829` | Require positive, negative and migration classes in the normative corpus | `src/riopa_provenance/canonical.py`, `tests/test_conformance.py` | Unknown or incomplete case classes fail before parity; external standards acceptance remains open |
| `WP006-OPENLINEAGE-PROJECTION-VALIDATION-20260829` | Reject incomplete OpenLineage run, job, dataset and producer/schema envelopes | `src/riopa_provenance/crate.py`, `tests/test_standards_roundtrip_contract.py`, `docs/wp006-openlineage-projection-validation-20260829.json` | Negative contract tests pass; external producer/consumer acceptance remains open |
| `WP006-DSSE-SUBJECT-DIGEST-VALIDATION-20260829` | Reject malformed subject digests before building unsigned DSSE/in-toto envelopes | `src/riopa_provenance/attestation.py`, `tests/test_attestation.py`, `docs/wp006-dsse-subject-digest-validation-20260829.json` | Lowercase 64-character SHA-256 contract is enforced; signing and external acceptance remain open |
| `WP006-PROV-IDENTITY-VALIDATION-20260829` | Reject empty PROV graph identifiers in the bounded projection validator | `src/riopa_provenance/crate.py`, `tests/test_standards_roundtrip_contract.py`, `docs/wp006-prov-identity-validation-20260829.json` | Non-empty graph identity is enforced; full PROV/SHACL and external acceptance remain open |
| `WP006-MATRIX-TRACEABILITY-20260829` | Reconcile the compatibility evidence index and verification guide with the observed successor matrix | `docs/ontology/interoperability-compatibility-matrix-20260829.json`, `docs/wp006-matrix-traceability-20260829.json`, `docs/conformance-and-release-verification.md` | Seven bounded Python/Node/Rust cases are observed; external standards acceptance and stable-release gates remain open |
