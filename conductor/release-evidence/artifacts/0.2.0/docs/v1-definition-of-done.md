# Stable v1.0 definition of done

RIOPA v1.0 is complete only when every condition below is evidenced. Feature completion or a successful candidate build is insufficient.
...
- Schemas, ontology, identifiers, APIs, CLI, configuration and file contracts are frozen and tagged.
- Python and at least one independent non-Python implementation pass the same positive and negative conformance corpus.
- Version negotiation, backward compatibility, migrations, rollback and deprecation rules pass.
- Existing repository adapters preserve native evidence and publish any semantic loss.
...
## Provenance, publication and preservation

- Every referenced record and bundled payload is schema-validated and integrity checked.
- Lineage queries answer source, transformation, downstream-impact and rebuild questions at declared granularity.
- Research objects include methods, citation, rights, quality, provenance, environment, SBOM, attestations and preservation metadata.
- Stable artifacts are signed, checksummed, cited and replicated in independent preservation failure domains.
...
- Accessibility, facility reconciliation, optimisation and simulation pass public benchmarks and independent verification.
- Objectives, constraints, uncertainty, equity, solver/simulation status and verification are inspectable.
